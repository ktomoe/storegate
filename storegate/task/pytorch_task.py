...
class PytorchTask(DLTask):
    ...
    def step_epoch(self, epoch, phase, dataloader):
        """Process model for given epoch and phase."""
        epoch_metric = EpochMetric(self._metrics, True, self._ml)
        num_batches = len(dataloader)
        pbar_args = dict(total=num_batches, disable=False)
    ...
    def step_batch(self, data, phase):
        ...
        rtn_result = {'batch_size': util.inputs_size(inputs)}
    ...
    def _output_to_storegate(self, outputs):
